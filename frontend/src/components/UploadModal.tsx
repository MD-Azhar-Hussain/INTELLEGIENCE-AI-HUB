...
import { X, Upload, AlertCircle, Loader2, FileText, Fingerprint } from "lucide-react";
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (data.articles) {
        setStatus("success");
        setTimeout(() => {
            onUploadSuccess(data.articles, file.name);
            onClose();
            setFile(null);
            setStatus("idle");
        }, 1000);
      ...
      {isOpen && (
        <div className="fixed inset-0 z-[160] flex items-center justify-center p-6">
          ...
          <motion.div 
            ...
            className="relative z-10 w-full max-w-xl bg-white dark:bg-[#111112] rounded-[30px] md:rounded-[40px] p-8 md:p-16 border-2 border-gray-100 dark:border-white/5 shadow-2xl overflow-hidden"
          >
            ...
            <div className="space-y-8 md:space-y-10">
              {!file ? (
                <div className="group relative">
                  <input
                    ...
                    accept=".pdf"
                  />
                  <div className="border-2 border-dashed border-gray-100 dark:border-white/10 rounded-2xl md:rounded-3xl py-16 md:py-24 flex flex-col items-center justify-center gap-3 md:gap-4 hover:border-blue-600 hover:bg-blue-50/50 transition-all">
              ...
              {status === "error" && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="p-4 rounded-xl bg-red-500/10 border border-red-500/20 text-red-500 text-[11px] md:text-sm font-bold flex gap-3">
